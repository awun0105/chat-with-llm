    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-1.5 pb-6">
          <a
            href="/"
            className="flex items-center gap-2.5 text-[1.15rem] font-bold tracking-[-0.04em] text-ink no-underline"
            aria-label="Chat with LLM home"
          >
            <span className="flex h-[26px] w-[26px] items-end gap-0.5 rounded-lg bg-ink p-[5px]" aria-hidden="true">
        ...
        <button
          type="button"
          className="flex min-h-[52px] w-full cursor-pointer items-center gap-3 rounded-[14px] border border-line-strong bg-white/70 px-3.5 text-[0.95rem] font-semibold transition duration-160 hover:-translate-y-px hover:border-[#c7c8c1] hover:bg-white"
          onClick={onNewChat}
        >
          <IconPlus />
          <span>New conversation</span>
          <kbd className="ml-auto font-mono text-[0.72rem] font-normal text-soft">⌘ K</kbd>
        </button>
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto" aria-live="polite">
          {sessions.map((session) => (
            <button
              ...
              type="button"
              className={[
                "my-0.5 w-full cursor-pointer rounded-xl border-0 px-[13px] py-3 text-left transition-colors duration-140",
        ...
        <div className="border-t border-line pt-3">
          <div className="flex items-center gap-2 px-2.5 py-1.5 pb-3 text-[0.8rem] text-muted">
            <span
              className={[
