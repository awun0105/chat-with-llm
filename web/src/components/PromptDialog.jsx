...
export function PromptDialog({ open, value, onChange, onClose, onSave }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[rgb(25_26_24_/_30%)] backdrop-blur-[3px]">
      <form
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-1 text-[1.4rem] font-semibold tracking-[-0.04em]">System instructions</h2>
          </div>
          <button
            type="button"
            className="grid size-[38px] place-items-center rounded-[10px] bg-transparent hover:bg-panel"
            onClick={onClose}
            aria-label="Close"
          >
            <IconClose />
        ...
        <label htmlFor="systemPrompt" className="mb-2 block text-[0.85rem] font-bold">
        ...
        <div className="mt-2 flex items-center justify-between font-mono text-[0.75rem] text-soft">
          <button
            type="button"
            className="cursor-pointer p-0 text-[0.82rem] font-bold text-accent"
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          <button
            type="button"
            className="cursor-pointer rounded-[10px] border border-line bg-white px-4 py-2.5 text-[0.88rem] font-bold"
            onClick={onClose}
          >
            Cancel
          </button>
          <button
            type="submit"
            className="cursor-pointer rounded-[10px] bg-ink px-4 py-2.5 text-[0.88rem] font-bold text-white"
          >
            Save instructions
          </button>
        </div>
